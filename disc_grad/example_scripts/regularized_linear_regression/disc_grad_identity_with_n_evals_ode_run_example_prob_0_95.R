...
grhmc_disc_grad_run <- foreach::foreach(l = 1:n_iterations) %dopar% {
  ...
  warmup_beta_plus_initial <- ifelse(og_regmod$coefficients[-1] < 0, mu, abs(og_regmod$coefficients[-1])) # start at MLE
  warmup_beta_minus_initial <- ifelse(og_regmod$coefficients[-1] > 0, mu, abs(og_regmod$coefficients[-1]))
  warmup_qbar_initial <- c(warmup_beta_plus_initial, warmup_beta_minus_initial, sum((og_regmod$residuals)^2) / og_regmod$df.residual)
  warmup_pbar_initial <- rnorm(grhmc_model_list$n_parameters)
  
  grad_indices <<- grhmc_model_list$grad_jump_fun(warmup_qbar_initial)
  ...
  warmup_boston_scale_fit_grhmc <- grhmc_discontinuous_gradient_transformed_function(
    model_list = grhmc_model_list,
    lambda = 0.2,
    T = time_period,
    n_samples = n_samples_per_iteration / 20,
    diag_s_elements_initial = rep(1, grhmc_model_list$n_parameters),
    m_initial = rep(0, grhmc_model_list$n_parameters),
    qbar_initial = warmup_qbar_initial,
    pbar_initial = warmup_pbar_initial,
    Lambda_initial = 0,
    u_initial = rexp(1),
    random_state = NULL,
    rtol = 1e-4,
  ...
  sampling_qbar_initial <- tail(warmup_boston_scale_fit_grhmc$q_original_samples, 1)
  sampling_pbar_initial <- rnorm(grhmc_model_list$n_parameters)
  
  sampling_boston_scale_fit_grhmc <- grhmc_discontinuous_gradient_transformed_function(
    model_list = grhmc_model_list,
    lambda = 0.2,
    T = time_period,
    ...
    diag_s_elements_initial = rep(1, grhmc_model_list$n_parameters),
    m_initial = rep(0, grhmc_model_list$n_parameters),
    qbar_initial = sampling_qbar_initial,
    pbar_initial = sampling_pbar_initial,
    Lambda_initial = 0,
    u_initial = rexp(1),
    random_state = NULL,
    rtol = 1e-4,
  ...
  list(
    q_original_samples = sampling_boston_scale_fit_grhmc$q_original_samples,
    n_evals_ode = sampling_boston_scale_fit_grhmc$n_evals_ode
  )
...
for (i in 1:n_iterations) {
  if (i == 1) {
    samples_matrix <- grhmc_disc_grad_run[[1]]$q_original_samples
  } else {
    samples_matrix <- rbind(samples_matrix, grhmc_disc_grad_run[[i]]$q_original_samples)
  }
}
