...
library(foreach)
source("disc_grad/implementation_scripts/VARI/full_VARI_grhmc_discontinuous_gradient_transformed_function.R")
...
n_adaptive_samples_per_iteration <- 1000
adaptive_time_period_per_iteration <- 1000
...
grhmc_disc_grad_run <- foreach::foreach(l = 1:n_iterations, .combine = "rbind") %dopar% {
  ...
  sink(paste0("disc_grad/example_scripts/section_6/bayesian_neural_network/log/log_nr_", l, ".txt"))
  system.time(
    neural_network_run <- VARI_grhmc_discontinuous_gradient_transformed_function(
  ...
  sink()
  list(
    samples_matrix = cbind(neural_network_run$sample_step_run$q_original_samples, rep(l, n_samples_per_iteration)),
    n_evals_ode = neural_network_run$sample_step_run$n_evals_ode
  )
...
# saveRDS(grhmc_disc_grad_run, "disc_grad/example_scripts/section_6/bayesian_neural_network/bnn_disc_grad_run_VARI_with_n_evals_ode_run_example.RDS")
grhmc_disc_grad_run <- readRDS("disc_grad/example_scripts/section_6/bayesian_neural_network/bnn_disc_grad_run_VARI_with_n_evals_ode_run_example.RDS")

for (i in 1:n_iterations) {
  if (i == 1) {
    samples_matrix <- grhmc_disc_grad_run[[1]]
  } else {
    samples_matrix <- rbind(samples_matrix, grhmc_disc_grad_run[[i]])
  }
}
