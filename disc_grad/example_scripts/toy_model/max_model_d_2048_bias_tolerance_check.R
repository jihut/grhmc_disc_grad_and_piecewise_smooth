...
library(dplyr)
library(ggplot2)
source("disc_grad/implementation_scripts/general_scripts/grhmc_discontinuous_gradient_transformed_with_relevant_indices_function.R")
...
for (k in 1:length(integrator_tol)) {
  ...
  store_chains <- foreach::foreach(i = 1:n_trajectories) %dopar% {
    ...
    test_run_warmup <- grhmc_discontinuous_gradient_transformed_function(
      ...
      relevant_indices = 1:3 # only keep the first ten coordinates as safety, the remaining should be the same as q2 anyways
    )
...
# saveRDS(
#   list(
#     list_chains_tol_scenario = list_chains_tol_scenario,
#     integrator_tol = integrator_tol
#   ),
#   "disc_grad/example_scripts/toy_model/list_max_model_d_2048_bias_tolerance_check.RDS"
# )

list_max_model <- readRDS("disc_grad/example_scripts/toy_model/list_max_model_d_2048_bias_tolerance_check.RDS")
list_chains_tol_scenario <- list_max_model$list_chains_tol_scenario
...
points(
  integrator_tol,
  sapply(1:4, function(i) mean(diff_result_matrix$q1_squared_plus_q2_squared[diff_result_matrix$int_tol == integrator_tol[i]])),
  col = "red",
  pch = 15
)
segments(
  integrator_tol,
  sapply(1:4, function(i) mean(diff_result_matrix$q1_squared_plus_q2_squared[diff_result_matrix$int_tol == integrator_tol[i]]) -
           sd(diff_result_matrix$q1_squared_plus_q2_squared[diff_result_matrix$int_tol == integrator_tol[i]]) * 1.96 / sqrt(n_trajectories)),
  integrator_tol,
  sapply(1:4, function(i) mean(diff_result_matrix$q1_squared_plus_q2_squared[diff_result_matrix$int_tol == integrator_tol[i]]) +
           sd(diff_result_matrix$q1_squared_plus_q2_squared[diff_result_matrix$int_tol == integrator_tol[i]]) * 1.96 / sqrt(n_trajectories)),
  col = "red"
