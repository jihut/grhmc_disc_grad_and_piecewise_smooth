...
for (k in 1:length(dim_vec)) {
  ...
  n_dim <- dim_vec[k]
  grhmc_run <- readRDS(
    paste0("disc_grad/example_scripts/toy_model/grhmc/max_model_d_", n_dim, "_grhmc.RDS")
  )
  relevant_matrix <- cbind(do.call(rbind, lapply(grhmc_run, estimated_quantities)), 
                           dim = n_dim)
  relevant_samples_array <- array(dim = c(n_samples, n_chains, 2))
  individual_ess_matrix <- matrix(nrow = n_chains, ncol = 4)
  individual_ess_matrix[, 4] <- n_dim
  for (l in 1:n_chains) {
    relevant_samples_array[, l, ] <- grhmc_run[[l]]$q_original_samples[, 1:2]
    individual_samples_array <- array(dim = c(n_samples, 1, 2)) 
    individual_samples_array[, 1, ] <- grhmc_run[[l]]$q_original_samples[, 1:2]
    ...
    individual_ess_matrix[l, 3] <- grhmc_run[[l]]$n_evals_ode
  }
  grhmc_monitor <- rstan::monitor(relevant_samples_array, warmup = 0)
  ess_matrix <- c(
    q = grhmc_monitor$n_eff[1:2], 
    n_evals_ode = sum(sapply(1:n_chains, function(i) grhmc_run[[i]]$n_evals_ode))
  )
  if (k == 1) {
...
saveRDS(list(final_result_matrix = final_result_matrix, diff_result_matrix = diff_result_matrix, final_ess_matrix = final_ess_matrix, final_individual_ess_matrix = final_individual_ess_matrix),
        "disc_grad/example_scripts/toy_model/max_model_varying_dimensions_grhmc_results.RDS")
grhmc_results <- readRDS("disc_grad/example_scripts/toy_model/max_model_varying_dimensions_grhmc_results.RDS")
final_result_matrix <- grhmc_results$final_result_matrix
final_ess_matrix <- grhmc_results$final_ess_matrix
diff_result_matrix <- grhmc_results$diff_result_matrix
