...
deSolverRoot <- function(y,
  ...
  p_old <- z[(3 + d):(3 + 2 * d - 1)]
  q_old <- z[3:(3 + d - 1)]
  old_grad <- log_target_grad(q_old)
  k1 <- c(lambda, 0, p_old, old_grad)
  ...
  while (t < Tmax) {
    ...
    while (not.done && ntrials < 20L) {
      ...
      p_old <- z[(3 + d):(3 + 2 * d - 1)]
      q_old <- z[3:(3 + d - 1)]
      old_grad <- k1[(3 + d):(3 + 2 * d - 1)]
      p_new <- p_old + h * old_grad / 2
      q_new <- q_old + h * p_new
      new_grad <- log_target_grad(q_new)
      p_new <- p_new + h * new_grad / 2
      
      z.new <- c(Lambda_old + h * lambda, z[2], q_new, p_new)
      k4 <- c(lambda, 0, p_new, new_grad)
      
      q_intermediate <- 0.5 * (q_new + q_old) + 
        h / 8 * (p_old - p_new)
      
      q_decoy <- q_old + h * p_old + 
        (h ^ 2) * (1 / 3 * old_grad + 1 / 6 * new_grad)
      
      grad_at_q_decoy <- log_target_grad(q_decoy)
      ...
      z.high <- c(z.new[1:2], q_decoy, p_decoy)
      
      err <-
        max(abs(z.new - z.high) / (atol + rtol * pmax(abs(z), abs(z.new))))
