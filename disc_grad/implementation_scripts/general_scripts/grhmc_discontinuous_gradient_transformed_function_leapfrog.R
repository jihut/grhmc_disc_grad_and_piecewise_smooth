source("disc_grad/implementation_scripts/general_scripts/deSolverRoot_leapfrog.R")
...
grhmc_discontinuous_gradient_transformed_function <- function(
  ...
  # state[1]: Lambda, which resets after each event
  # state[2]: u, simulate from exp(1) after each event
  # state[3:(3 + n_dim - 1)]: qbar
  # state[(3 + n_dim):(3 + 2*n_dim - 1)]: pbar
  ...
  ode <- function(t, state, parms){
    ...
    ret <- c(
      ...
      0, # u
      state[(3 + d):(3 + 2 * d - 1)], # \dot qbar = pbar
      diag_s_elements_initial *
        model_list$log_target_grad(m_initial + diag_s_elements_initial * state[3:(3 + d - 1)]) #\dot pbar = gradient of log transformed density wrt. qbar
    )
  ...
  region_lin_root_fun <- function(t, y, parms) {
    ...
    if (is.null(additional_lin_root_list)) { # if no additional linear root functions are given
      ...
      if (num_of_lin_constraints == 0) {
        ...
        lin_constraints_root_fun <- c(diag_s_elements_initial * model_list$region_lin_root_list$A) %*% y[3:(3 + d - 1)] + model_list$region_lin_root_list$A %*% m_initial + model_list$region_lin_root_list$B
        ...
        lin_constraints_root_fun <- as.numeric(Matrix::t(sparse_diag_s %*% transposed_model_list_linear_constrained_A) %*% y[3:(3 + d - 1)]) + as.numeric(model_list_linear_constrained_A %*% m_initial) + model_list$region_lin_root_list$B
        
      }
      
      c(
        y[1] - y[2], # momentum refresh 
      ...
      if (num_of_lin_constraints == 0) {
        ...
        lin_constraints_root_fun <- c(diag_s_elements_initial * transposed_model_list_linear_constrained_A) %*% y[3:(3 + d - 1)] + model_list$region_lin_root_list$A %*% m_initial + model_list$region_lin_root_list$B
      ...
      if (num_of_additional_lin_eqs == 1) {
        ...
        additional_lin_root_fun <- c(diag_s_elements_initial * transposed_additional_lin_root_A) %*% y[3:(3 + d - 1)] + additional_lin_root_list$A %*% m_initial + additional_lin_root_list$B
        
      } else {
        
        # lin_constraints_root_fun <- t(diag(diag_s_elements_initial) %*% transposed_model_list_linear_constrained_A) %*% y[4:(4 + d - 1)] + model_list$region_lin_root_list$A %*% m_initial + model_list$region_lin_root_list$B
        additional_lin_root_fun <- as.numeric(Matrix::t(sparse_diag_s %*% transposed_additional_lin_root_A) %*% y[3:(3 + d - 1)]) + as.numeric(additional_lin_root_A %*% m_initial) + additional_lin_root_list$B
  ...
  sim_out <- deSolverRoot(
    ...
    log_target_grad = model_list$log_target_grad,
    lambda = lambda, 
    d = d, 
  ...
  q_original_samples <- t(m_initial + diag(diag_s_elements_initial, nrow = d) %*% t(df_sim_out[, 3:(3 + d - 1) + 1]))[-1, ]
