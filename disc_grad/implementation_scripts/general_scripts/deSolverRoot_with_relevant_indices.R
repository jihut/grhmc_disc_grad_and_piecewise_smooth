...
deSolverRoot <- function(y,
  ...
  relevant_indices_null <- is.null(relevant_indices)
  length_relevant_indices <- length(relevant_indices)
  if (relevant_indices_null) {
    D <- length(y)
  } else {
    D <- length_relevant_indices
  }
  z <- y
  ...
  if (relevant_indices_null) {
    ...
    samples[1, 2:(D + 1)] <- y[relevant_indices]
  }
  samples.count <- 2L
  ...
  while (t < Tmax) {
    ...
    while (t + h * last  >= s.times[samples.count] - 1.0e-14) {
      ...
      if (relevant_indices_null) {
        ...
        samples[samples.count, 2:(D + 1)] <- z[relevant_indices] + 
          r * ff1[relevant_indices] + 
          r ^ 2 * ff2[relevant_indices] + 
          r ^ 3 * ff3[relevant_indices]
      }
  ...
  # }
  return(list(samples = samples, n.evals = n.evals, z_final = z))
}
