...
VARI_adaptive_step_grhmc_discontinuous_gradient_transformed_function <- function( # tune both lambda (event rate), m and diagonal S at the same time
  model_list,
  lambda,
    ...
    if (t > 0) {
      ...
      if (whichroot == 1) { # if root due to first root function --> momentum refresh
        ...
        if (t > proportion_time_until_adaptive_start * T) { # adaptive period --> update s, m and lambda
          ...
          s_adaptive <<-
            pmin(
              max_proportion_of_previous_state * s_adaptive,
              pmax(
                min_proportion_of_previous_state * s_adaptive,
                proposed_s_adaptive
              )
            )
