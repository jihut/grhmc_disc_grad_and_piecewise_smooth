# Edited version of deSolverRoot_fixed_step_size in the illustration_global_error example. 
# deSolverRoot now will try to store the states at the end of each integration step. 
...
deSolverRoot <- function(y,
  ...
  z_end <- matrix(NA, ceiling(Tmax / h) + 1, D)
  z_end_count <- 0
  t <- times[1]
  samples[1, 1] <- t
  samples[1, 2:(D + 1)] <- y
  ...
  while (t < Tmax) {
    ...
    largest_time_ratio <- min(1, (Tmax - t) / h)
    ...
    if (curr.root.obj$which.root > 0 & largest_time_ratio > curr.root.obj$root.time) {
    ...
    } else if (largest_time_ratio < 1) {
      last <- largest_time_ratio
      z.new <- z + last * ff1 + last ^ 2 * ff2 + last ^ 3 * ff3
    }
    ...
    t <- t + h * last
    z <- z.new
    z_end_count <- z_end_count + 1
    z_end[z_end_count, ] <- z.new
  ...
  # }
  return(list(samples = samples, n.evals = n.evals, z_end = z_end))
}
