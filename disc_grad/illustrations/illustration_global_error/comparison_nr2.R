...
# Consider q1 ~ N(0, 1), q2 ~ N(max(0, c * q1), 1) with c = 0.1.
...
h_vec_c_0_1 <- h_vec
leapfrog_store_error_c_0_1 <- leapfrog_store_error
leapfrog_store_matrix_c_0_1 <- leapfrog_store_matrix
...
plot(h_vec_c_0_1, leapfrog_store_error_c_0_1, log = "xy", ylim = c(1e-11, 1), ylab = "L2 error at final state", xlab = "h (step size)", main = "c = 0.1")
...
final_store_matrix_c_0_1 <- rbind(
  leapfrog_store_matrix_c_0_1, 
  grhmc_nr1_store_matrix_c_0_1,
  grhmc_nr2_store_matrix_c_0_1
)

final_store_matrix_c_0_1 %>%
  ggplot(aes(x = h, y = l2_error)) + 
  geom_point(aes(colour = method, shape = method)) + 
  scale_shape_manual(values = c(1, 3, 1), guide = "none") + 
  geom_function(aes(colour = "Line"), fun = function(x) 0.068 * x ^ 3, linetype = 2) + 
  scale_color_manual(
