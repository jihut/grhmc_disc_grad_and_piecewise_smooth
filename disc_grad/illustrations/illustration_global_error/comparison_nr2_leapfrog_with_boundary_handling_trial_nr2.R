...
leapfrog_nr4 <- function(grad_U, h, L, q_initial, p_initial) {
  ...
  for (i in 1:L) {
    # print("i")
    # print(i)
    # The below corresponds to the following:
    # Instead of the mismatch between using position update based on the first momentum update using the given full step size h and then using this to approximate t_hit
    # such that in version 2, for large step size, using the t_hit from the above to perform a full leapfrog update might lead to q that that is might not be very close to zero
    # because the time hitting the boundary based on the momentum update based on t_hit from the above is different from t_hit itself.
    # To fix this issue, one could find a step size tau that connects with the first momentum update and also corresponds to the time to hit based on the mentioned obtained momentum as well. 
    # I.e. find tau such that the first momentum update is p' = p - tau / 2 * grad(q)
    # And also such that q' = q + p' * tau is such that q'[1] = 0.
    # This gives q[1] + p[1] * tau - grad(q)[1] / 2 * tau ^ 2 = 0 and one needs to solve for tau. 
    root_fun <- c(polyroot(c(current_q[1], current_p[1], -grad_U(current_q)[1] / 2)))
    # print(root_fun)
    t_hit <- min(abs(Re(root_fun))) # find the smallest value of the real root
    t_hit_index <- which.min(abs(Re(root_fun)))
    if (t_hit < h & Re(root_fun[t_hit_index]) > 0) { # if the root is positive and is smaller than the large step size h --> boundary collision during the full step h
      print("boundary hitting!")
      print("current q")
      print(current_q)
      print("t hit")
      print(t_hit)
      new_p <- current_p - t_hit / 2 * grad_U(current_q) # standard half step update of momentum first
      new_q <- current_q + t_hit * new_p # standard full step update of position
      ...
      new_p <- current_p - h / 2 * grad_U(current_q)
...
leapfrog_nr4_grad_U <- function(q) { # U = negative log target, therefore negative log gradient here
  if (q[1] <= 0) {
    -grad_nr1(q)
  } else {
    -grad_nr2(q)
...
leapfrog_nr4_output <- function(grad_U, time_period, q_initial, p_initial, z_true, h) {
  ...
  run_leapfrog <- leapfrog_nr4(grad_U, h, L, q_initial, p_initial)
...
leapfrog_nr4_output(grad_U = leapfrog_nr4_grad_U, time_period = time_period, q_initial = q_initial, p_initial = p_initial, z_true = z_true, h = 0.01)  
leapfrog_nr4_output(grad_U = leapfrog_nr4_grad_U, time_period = time_period, q_initial = q_initial, p_initial = p_initial, z_true = z_true, h = 0.005)  

# Run for a range of values for the step size h

leapfrog_nr4_store_error <- numeric(length(h_vec))

for (i in 1:length(h_vec)) {
  
  leapfrog_nr4_store_error[i] <- leapfrog_nr4_output(grad_U = leapfrog_nr4_grad_U, time_period = time_period, q_initial = q_initial, p_initial = p_initial, z_true = z_true, h = h_vec[i])$l2_error  
  
}

leapfrog_nr4_store_matrix <- data.frame(method = rep("Leapfrog", length(h_vec)), h = h_vec, l2_error = leapfrog_nr4_store_error)
...
summary(lm(l2_error ~ I(h ^ 2) + h, data = leapfrog_nr4_store_matrix))
range(c(leapfrog_nr1_store_error, leapfrog_nr4_store_error, grhmc_nr1_store_error, grhmc_nr2_store_error))

h_vec_c_0_1 <- h_vec
leapfrog_nr1_store_error_c_0_1 <- leapfrog_nr1_store_error
leapfrog_nr1_store_matrix_c_0_1 <- leapfrog_nr1_store_matrix
leapfrog_nr4_store_error_c_0_1 <- leapfrog_nr4_store_error
leapfrog_nr4_store_matrix_c_0_1 <- leapfrog_nr4_store_matrix
grhmc_nr1_store_error_c_0_1 <- grhmc_nr1_store_error
grhmc_nr1_store_matrix_c_0_1 <- grhmc_nr1_store_matrix
grhmc_nr2_store_error_c_0_1 <- grhmc_nr2_store_error
grhmc_nr2_store_matrix_c_0_1 <- grhmc_nr2_store_matrix

plot(h_vec_c_0_1, leapfrog_nr1_store_error_c_0_1, log = "xy", ylim = c(1e-11, 1), 
     ylab = "L2 error at final state", xlab = "h (step size)", main = "c = 0.1", cex = 0.25,
     col = adjustcolor("black", alpha.f = 1))
points(h_vec_c_0_1, leapfrog_nr4_store_error_c_0_1, col = adjustcolor("orange", alpha.f = 1), cex = 0.25)
points(h_vec_c_0_1, grhmc_nr1_store_error_c_0_1, col = adjustcolor("red", alpha.f = 1), cex = 0.25)
points(h_vec_c_0_1, grhmc_nr2_store_error_c_0_1, col = adjustcolor("blue", alpha.f = 1), pch = 3, cex = 0.25)
lines(h_vec_c_0_1, 0.068 * h_vec_c_0_1 ^ 3, lty = 2, col = "green")
lines(h_vec_c_0_1, 0.21 * h_vec_c_0_1 ^ 2, lty = 2, col = "darkgreen")
legend(
  "bottomright",
  legend = c(
    "Standard leapfrog",
