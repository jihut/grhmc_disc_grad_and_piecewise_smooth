...
# Here: All coordinates follow a spike and slab distribution
...
grhmc_model_list$weight_slab <- rep(0.25, (grhmc_model_list$n_parameters))
grhmc_model_list$sigma_slab <- rep(sigma_slab, grhmc_model_list$n_parameters)

grhmc_model_list$log_target_grad <- function(q) { # new version, vectorized and therefore slightly faster
  
  prior_beta_term <- -q / grhmc_model_list$sigma_slab ^ 2
