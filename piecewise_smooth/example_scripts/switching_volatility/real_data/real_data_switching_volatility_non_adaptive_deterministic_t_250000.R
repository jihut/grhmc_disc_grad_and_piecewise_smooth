...
final_run <- foreach::foreach(l = 1:10) %dopar% {
  ...
  print("warmup")
  system.time(
    warmup_run <- grhmc_piecewise_smooth_density_transformed_function(
  ...
  set.seed(as.numeric(paste0(l, l)))
  
  sampling_qbar_initial <- warmup_run$output_from_ode_solver[nrow(warmup_run$output_from_ode_solver), colnames(warmup_run$output_from_ode_solver) %in% paste0("qbar", 1:grhmc_model_list$n_parameters)]
  ...
  print("sampling")
  system.time(
    sampling_run <- grhmc_piecewise_smooth_density_transformed_function(
      model_list = grhmc_model_list,
      lambda = 0.2,
      T = 250000,
      n_samples = 5000000,
...
final_run <- lapply(final_run, function(x) { # reduce memory issue if the full output is not required
  x["output_from_ode_solver"] <- NULL
  x
})
...
final_samples_3d_array <- abind::abind(lapply(final_run, '[[', "q_original_samples"), along = 3)
...
store_matrix <- cbind(final_result$q_original_samples, chain_index = sort(rep(1:10, 50000)))
