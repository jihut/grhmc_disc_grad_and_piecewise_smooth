...
store_matrix <- foreach::foreach(l = 1:10, .combine = "rbind") %dopar% {
  ...
  if (l <= 5) {
    ...
    z_initial <- rnorm(num_time_points, mean = 1) # an assumption that the majority of the time points are in the high volatility state
  }
...
colnames(df_ggplot)[5:6] <- c("posterior_quantile_z_2_5", "posterior_quantile_z_97_5")
...
plot_nr2 <- df_ggplot %>%
  select(t, posterior_mean_z, true_z) %>%
  pivot_longer(
    cols = !t, 
    names_to = "z_type",
    values_to = "z_value"
  ...
  ggplot() + 
  geom_line(aes(x = t, y = z_value, col = z_type)) + 
  geom_ribbon(data = df_ggplot, aes(x = t, ymin = posterior_quantile_z_2_5, ymax = posterior_quantile_z_97_5), alpha = 0.15, fill = "#00BFC4", linetype = 0) + 
  scale_color_manual(labels = c("Posterior Mean", "True"), values = c("#00BFC4", "#F8766D")) +
  labs(y = expression(Z[t]), color = "")
plot_nr2
