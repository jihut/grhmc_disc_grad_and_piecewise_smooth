...
store_matrix <- foreach::foreach(i = 1:1000, .combine = rbind) %dopar% {
  # sink(paste0("piecewise_smooth/example_scripts/toy_model/log_randomized/log_nr", i, ".txt"))
  set.seed(i)
  c1 <- exp(-1 / 2)
  c2 <- exp(-1 / 8) # both are normalising constant in order to get 1 when integrating over the whole R^2. 
...
# saveRDS(store_matrix, "piecewise_smooth/example_scripts/toy_model/normal_2d_unit_circle_randomized_reflection_non_adaptive_lambda_0_2.RDS")
store_matrix <- readRDS("piecewise_smooth/example_scripts/toy_model/normal_2d_unit_circle_randomized_reflection_non_adaptive_lambda_0_2.RDS")
