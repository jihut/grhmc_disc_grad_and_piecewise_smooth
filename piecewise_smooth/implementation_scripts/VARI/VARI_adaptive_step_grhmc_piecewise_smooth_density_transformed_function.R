...
VARI_adaptive_step_grhmc_piecewise_smooth_density_transformed_function <- function(
    model_list,
    lambda,
  ...
  ode <- function(t, y, parms){
    ...
    ret <- c(
      ...
      s_adaptive *
        model_list$log_target_grad(m_adaptive + s_adaptive * y[4:(4 + d - 1)]), #\dot pbar = gradient of log transformed density wrt. qbar
  ...
  region_lin_root_fun <- function(t, y, parms) {
    ...
    if (is.null(additional_lin_root_list)) { # if no additional linear root functions are given
      ...
      if (num_of_lin_constraints == 0) {
        ...
        lin_constraints_root_fun <- (s_adaptive * model_list$region_lin_root_list$A) %*% y[4:(4 + d - 1)] + model_list$region_lin_root_list$A %*% m_adaptive + model_list$region_lin_root_list$B
      ...
      if (num_of_additional_lin_eqs == 1) {
        ...
        additional_lin_root_fun <- (s_adaptive * additional_lin_root_list$A) %*% y[4:(4 + d - 1)] + additional_lin_root_list$A %*% m_adaptive + additional_lin_root_list$B
    ...
    if (t > 0) {
      ...
      if (whichroot == 1) { # if root due to first root function --> momentum refresh
        ...
        if (t > proportion_time_until_adaptive_start * T) { # adaptive period --> update s, m and lambda
          ...
          proposed_s_adaptive <-
            sqrt(y[(4 + 5 * d):(4 + 6 * d - 1)] / t - m_adaptive ^ 2)
          ...
          # if (!nut_time_found_indicator & !reflection_indicator) {
          #   
          #   sum_all_nut_times <<- sum_all_nut_times + (t - t_at_current_qbar)
          #   
          # } else {
        ...
        old_q <- m_adaptive + s_adaptive * old_qbar
        ...
        model_list$target_jump_fun(m_adaptive + s_adaptive * new_qbar_eps) # make sure that one is in the new region
        ...
        if (num_of_lin_constraints != 1) {
          normal_vec <- s_adaptive * model_list$region_lin_root_list$A[whichroot - 1, ]
        } else {
          normal_vec <- s_adaptive * model_list$region_lin_root_list$A
        }
        ...
        if (norm_squared_old_pbar_perpendicular > 2 * delta_U) { # if the momentum squared along the normal direction is larger than two times the potential energy --> transition to new region
          ...
          model_list$target_jump_fun(old_qbar_eps * s_adaptive + m_adaptive) # ensure that the region just before hit is relevant again
          # print(paste0("New region: ", region_id))
          
          
          # sum_all_nut_times <<- sum_all_nut_times + (t - t_at_current_qbar)
          # n_uncensored_nut_times <<- n_uncensored_nut_times + 1
          # # current_qbar <<- old_qbar_eps
